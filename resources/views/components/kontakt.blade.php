<section class="flex flex-col lg:flex-row space-y-6 lg:space-y-0 space-x-0 lg:space-x-4 justify-between items-start px-6 py-20 bg-gray-300 text-slate-800">
    <section class="w-full lg:w-1/2 bg-gray-200 px-2 py-4 rounded">
        ...
        @else
        <h2 class="mb-3 text-2xl font-semibold">Upišite kontakt u sljedeću Formu:</h2>
        <p class="mb-6 font-semibold text-red-500">Znamo da želite odmah sve vidljive cijene. Ako nam pošaljete ovdje upit za nagradu dobivate dosta niže akcijske cijene od nekih vidljivih 
            cijena na našem webu, čim kliknete POŠALJI, na vaš e-mail će automatski (odmah) doći kompletan cjenik s dodatnim popustom  + montaža, 
            sve detaljno opisano (molimo da pripazite na točnost e-mail adrese). Radimo u cijeloj Hrvatskoj, te osim prodaje vršimo uslugu montaže i servisa.</p>
        @endif
